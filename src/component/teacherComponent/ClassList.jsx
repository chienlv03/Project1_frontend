...
const ClassList = () => {
    ...
    useEffect(() => {
        const fetchClassList = async () => {
            try {
                const response = await axios.get('http://localhost:8081/classrooms/all'); // Replace with your actual API endpoint
    ...
    const handleDeleteClass = async (classId) => {
        try {
            await axios.delete(`http://localhost:8081/classrooms/remove/${classId}`); // Replace with your actual API endpoint
    ...
    return (
        <div className="p-4">
            <div className='mt-16'>
                <button onClick={() => navigate('/add-class-form')} type="button" className="flex text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 
                ...
                <div className="grid grid-cols-4 gap-4 mb-4">
                    {classList.map(classItem =>
                        <div key={classItem.id} className="relative h-44 p-2 rounded bg-gray-50 dark:bg-blue-200">
                            <ul onClick={() => handleClassClick(`/detail-class/${classItem.id}/list-student`)} className='cursor-pointer'>
                            ...
                            <ul className='absolute z-20 my-2 h-10 flex right-0 top-0 bg-blue-300'>
                                <li onClick={() => navigate(`/edit-class-form/${classItem.id}`, { state: { class: classItem } })} className='cursor-pointer py-1 px-2 h-10 border-r-2'>
                                    <svg className="h-6 w-6 text-gray-800" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" 
